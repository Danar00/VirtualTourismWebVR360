    ...
    <a-scene canvas="" keyboard-shortcuts="" vr-mode-ui="">
      ...
      @foreach ($panoramas as $target)
        @foreach ($linkPanorama as $linked)
          @if ($linked->key_target == $target->key_panorama && $c < $counts)
            <a-entity id="links" position="{{$i}} {{$y}} -4" rotation="0 0 0">
              <a-plane color="#CCC" height="0.4" width="3" position="0 -1.2 0" ></a-plane>
              <a-text value="{{$kampungs[$c]->nama_tempat}}" position="0 -1.2 0" align="center" width="3" color="black" scale="2 2 2"></a-text>
              <a-entity template="src: #link" data-thumb="{{$kampungs[$c]->link_gambar}}" href="{{ url('/user-panorama/'.$target->nama_panorama) }}"></a-entity>
            </a-entity>
            @php
              $i+=4;
              $c++;
